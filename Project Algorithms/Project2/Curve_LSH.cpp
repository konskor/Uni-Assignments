...
//	int numbuckets = grid_points->size()/8;
	int numbuckets = grid_points->size()/16;	// for project2
...
/* For clustering */
void Curve_LSH::UpdateVector_Node(vector<Item*>* items){

	//link Vector_Node with the correct Item*

	for(unsigned int i = 0; i < grid_points->size(); i++)
		grid_points->at(i)->setItem(items->at(i));
}

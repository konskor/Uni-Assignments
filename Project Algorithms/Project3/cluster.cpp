...
bool Read_Input1(string filename, vector<Item*>* result);
...
//	bool curve = Read_Input(input_file,original_items);		// Different Input function for Project3
	bool curve = Read_Input1(input_file,original_items);
	
	
	cout << "Original size is " << original_items->size() << endl;
...
//	Remove_Duplicates(original_items);		//No need in Project3
...
//This is used for project 3 input (nn_representation.csv and new_representation.csv)
bool Read_Input1(string filename, vector<Item*>* result){
	...
	return false;	//reading Vectors, not curves
}

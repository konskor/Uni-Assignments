...
int main(int argc,char** argv){
	...
	int L = 4;		// how many LSH_HT tables for each Relevant traversal
	...
	for(int i = 0; i < argc; i++){
		...
		if( strcmp(argv[i] , "-L_vec") == 0 ){// how many LSH_HT for each traversal (optional)
...
while( end_flag == false){ 
	...
	cout << "Curves_size is " << curves_cut->size() << endl;
	cout << "Queries_size is " << queries_cut->size() << endl;	
...
//	int M = Curve_NN_True1(curves,queries,info);
...
/////////////////////////////////////////////////////
	...
	Relevant_Table* table = new Relevant_Table(M,d,window,e,true);
	
	cout << "Finished Creating the empty Relevant_Table\n";
	
	cout << "Started Inserting\n";
	
//	table->Insert(curves,k,L); 

	table->Insert(curves_cut,k,L); 

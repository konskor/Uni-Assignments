...
int main(int argc,char** argv){
	...
	int bh_d = 3;	// d' for HyperCube (dummy value , later logn)
	int M = 10;
	int probes = 2;
	
	int k = 4;	// the h(x) for LSH family
	
	bool bhd_input = false;	// if we read d' from input
	...
	for(int i = 0; i < argc; i++){
		...
		if( strcmp(argv[i] , "-M") == 0 ){		// M for HyperCube (optional)
		...
		if( strcmp(argv[i] , "-probes") == 0 ){		// probes for HyperCube (optional)
...
/////////////////////////////////////////////////////
	...
	for(int i = 0; i < L; i++){
		...
		if(bhd_input == false) bh_d = round( log(vn->size()) );
		
		cout << "Bh_d is " << bh_d << endl;
...
//		double w = 10;
		...
		grids[i]->setHTc(k,bh_d,M,probes,w);	// Creating the hasH Table	
		vn = NULL;
	}
...
//		double t1 = Timestamp();	
		...
		for( int j = 0; j < L; j++){	//for all Grids/HashTables
			...
			grids[j]->getHTc()->Search1(v1,info->at(i));	//Search NN , L_vec=1
...
//	total = sum/ ( L * times->size() );	
	// div L , because vec<>times contains each search from each grid (not as sum)
	// so the sum is bigger ==> div L
